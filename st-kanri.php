...
$stdata50 = get_option( 'st-data50' ); //メニューのワイド停止
...
function my_admin_menu() {
     ...
     add_submenu_page(
	  'my-custom-admin',
          __( 'AFFINGER管理リセット', 'my-custom-admin' ),
          __( 'AFFINGER管理リセット', 'my-custom-admin' ),
          'manage_options',
          'my-submenu',
          'my_submenu'
...
<p>※<a target="_blank" href="//manualstinger.com/mokuzi/" />利用マニュアルサイト<a><br/>※<a href="//manualstinger.com/plugin/" target="_blank" />推奨プラグイン等について</a></p>
		...
		<h4><i class="fa fa-th-list"></i>&nbsp;投稿一覧</h4>
               ...
               <p>
                    <input type="checkbox" name="st-data50" value="yes" <?php if ( $GLOBALS["stdata50"] === 'yes' ) {
                         echo 'checked';
                    } ?>>
                   画像をワイドにした場合もメニューはワイドにしない</p>
...
<p>メディアより画像をアップロードして「ファイルのURL」を記入して下さい。（例：http://example.com/wp-content/uploads/2015/11/favicon.ico ）</p>
		...
		<p>「<a href="//ja.wordpress.org/plugins/sns-count-cache/" target="_blank">SNS Count Cacheプラグイン</a>」を利用するとソーシャルカウントが表示されます（※Twitterは出ません）</p>
               ...
               <P>トップタイトルを書き替え（ここに記述したタイトルが優先されます）<br/>
                    <input type="text" name="st-data33" style="width:50%" placeholder="検索キーワードを含めた全角32文字以内が推奨されています" value="<?php echo esc_attr( $GLOBALS["stdata33"] ); ?>">
               </p>

               <P>トップ用のメタキーワード（複数ある場合は半角カンマ「,」で区切る）<br/>
                    <input type="text" placeholder="松,竹,梅" name="st-data46" style="width:50%" placeholder="松,竹,梅" value="<?php echo esc_attr( $GLOBALS["stdata46"] ); ?>">
               </p>
...
function my_submenu() {
     ...
     <div class="wrap">
          <h2>AFFINGER基本管理リセット画面</h2>
           <form id="my-main-form" method="post" action="">
               <?php wp_nonce_field( 'my-nonce-key', 'my-custom-admin' ); ?>
			<p style="color:#ff0000">※テーマを初めてセットした時及び「AFFINGER管理画面」に不具合が生じた場合、こちらからリセットできます。</p>
